...
`cargo run --release -- run` executes the same stages in order. Downloads use `DOWNLOAD_WORKERS` concurrent MassIVE transfers. Keep it at `1` for the initial production run, then raise it only after the restart behavior and network load look stable.
